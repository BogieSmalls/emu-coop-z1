...
def test_beta8_release_version_is_advertised():
    assert __version__ == "2.0 beta8"


def test_readme_and_release_notes_advertise_beta8_artifacts():
    readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
    release_notes = (REPO_ROOT / "RELEASE_NOTES.md").read_text(encoding="utf-8")

    for text in (readme, release_notes):
        assert "z1rr-coop-2.0-beta8" in text
        assert "emu-coop-plus-2.0-beta8" not in text
