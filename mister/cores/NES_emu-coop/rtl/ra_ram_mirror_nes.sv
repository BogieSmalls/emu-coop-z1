...
//
//   [0x58000] EmuCoop write control:
//             request_seq(8) + count(8) + ack_seq(8) + status(8) + reserved(32)
//   [0x58008] EmuCoop write pairs:
//             nes_addr(16) + value(8) + reserved(40), one pair per 64-bit word
...
module ra_ram_mirror_nes #(
	...
	// SDRAM read/write interface (ch2)
...
// EmuCoop ARM→FPGA write mailbox
localparam [28:0] WRITE_CTRL_ADDR = DDRAM_BASE + 29'hB000;  // byte offset 0x58000 / 8
localparam [28:0] WRITE_PAIR_BASE = DDRAM_BASE + 29'hB001;  // byte offset 0x58008 / 8
localparam [7:0]  MAX_WRITE_PAIRS = 8'd32;
localparam [7:0]  WRITE_STATUS_OK = 8'd0;
localparam [7:0]  WRITE_STATUS_INVALID_ADDR = 8'd1;
localparam [7:0]  WRITE_STATUS_TIMEOUT = 8'd2;
...
// EmuCoop write mailbox states
localparam S_WR_PARSE     = 5'd25;
localparam S_WR_RD_PAIR   = 5'd26;
localparam S_WR_APPLY     = 5'd27;
localparam S_WR_WAIT      = 5'd28;
localparam S_WR_NEXT      = 5'd29;
localparam S_WR_ACK       = 5'd30;
...
// EmuCoop write mailbox registers
reg  [7:0] wr_request_seq;
reg  [7:0] wr_last_seen_seq;
reg  [7:0] wr_count;
reg  [7:0] wr_idx;
reg  [7:0] wr_status;
...
// ======================================================================
always @(posedge clk) begin
	// Default: deassert SDRAM strobes (pulse-based)
	sdram_rd <= 1'b0;
	sdram_wr <= 1'b0;
	...
	if (reset) begin
		...
		S_IDLE: begin
			...
			if (vblank_pending) begin
				...
				// Poll EmuCoop writes first, then RTQuery if no write is pending.
				active <= 1'b1;
				qry_poll_timer <= 11'd0;
				ddram_rd_addr <= WRITE_CTRL_ADDR;
				ddram_rd_req  <= ~ddram_rd_req;
				return_state  <= S_WR_PARSE;
				state         <= S_DD_RD_WAIT;
			end
		end
		...
		// =============================================================
		S_WR_PARSE: begin
			if (rd_data[7:0] != wr_last_seen_seq &&
			    rd_data[15:8] != 8'd0 &&
			    rd_data[23:16] == 8'd0) begin
				wr_request_seq <= rd_data[7:0];
				wr_count       <= (rd_data[15:8] > MAX_WRITE_PAIRS) ?
				                  MAX_WRITE_PAIRS : rd_data[15:8];
				wr_idx         <= 8'd0;
				wr_status      <= WRITE_STATUS_OK;
				state          <= S_WR_RD_PAIR;
			end
		...
		S_WR_RD_PAIR: begin
			ddram_rd_addr <= WRITE_PAIR_BASE + {21'd0, wr_idx};
			ddram_rd_req  <= ~ddram_rd_req;
			return_state  <= S_WR_APPLY;
			state         <= S_DD_RD_WAIT;
		end

		S_WR_APPLY: begin
			if (rd_data[15:0] < 16'h2000) begin
				sdram_addr <= CPURAM_BASE + {14'd0, rd_data[10:0]};
				sdram_din  <= rd_data[23:16];
				sdram_wr   <= 1'b1;
				timeout    <= 16'd0;
				state      <= S_WR_WAIT;
			end
			else begin
				wr_status <= WRITE_STATUS_INVALID_ADDR;
				state     <= S_WR_NEXT;
			end
		end
		...
		S_WR_WAIT: begin
			...
			if (sdram_data_valid) begin
				...
				wr_status <= WRITE_STATUS_TIMEOUT;
				state     <= S_WR_NEXT;
			end
		end

		S_WR_NEXT: begin
			if (wr_idx + 8'd1 >= wr_count) begin
				state <= S_WR_ACK;
			end
			else begin
				wr_idx <= wr_idx + 8'd1;
		...
		S_WR_ACK: begin
			wr_last_seen_seq <= wr_request_seq;
			ddram_wr_addr    <= WRITE_CTRL_ADDR;
			ddram_wr_din     <= {32'd0, wr_status, wr_request_seq, wr_count, wr_request_seq};
			ddram_wr_be      <= 8'hFF;
			ddram_wr_req     <= ~ddram_wr_req;
			return_state     <= S_IDLE;
			state            <= S_DD_WR_WAIT;
